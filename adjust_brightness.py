...
def adjust_brightness_from_src_to_dst(dst, src,path=None,if_show=None, if_info=None):
    ...
    if path != None:
        cv2.imwrite(path, cv2.cvtColor(dstf, cv2.COLOR_BGR2RGB))

    return dstf

if __name__ == '__main__':

  A = read_img('pairs/1 (1).png')
  B = read_img('pairs/15.jpg')

  # adjusted = adjust_brightness_from_src_to_dst(A, B, path='resA.png')
  adjusted = adjust_brightness_from_src_to_dst(A, B,path='result.png')